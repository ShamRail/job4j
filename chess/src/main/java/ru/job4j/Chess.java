...
public class Chess extends Application {
    ...
    /**Строит фигуру.*/
    private Rectangle buildFigure(int x, int y, int size, String image) throws ImposibleMoveException {
        Rectangle rect = new Rectangle();
        rect.setX(x);
        rect.setY(y);
        rect.setHeight(size);
        ...
        rect.setOnMouseReleased(
                event -> {
                    try {
                        ...
                        this.showAlert(ime.getMessage());
                        rect.setX(((int) momento.getX() / 40) * 40 + 5);
                        rect.setY(((int) momento.getY() / 40) * 40 + 5);
                    } catch (FigureNotFoundException fnfe) {
                        this.showAlert(fnfe.getMessage());
                    } catch (OccupiedWayException owe) {
                        this.showAlert(owe.getMessage());
                        rect.setX(((int) momento.getX() / 40) * 40 + 5);
                        rect.setY(((int) momento.getY() / 40) * 40 + 5);
                    }
                }
    ...
    /**Вызывает окно и выдает сообщение пользователю, в случае исключения.*/
